...
public class SimpleProcessorBenchmarkClientRunnable implements ClientRunnable {
    ...
    public SimpleProcessorBenchmarkClientRunnable(String targetIP, int targetPort, int msgId,int requestSize ,int rpcTimeout,
    ...
    private void runJavaAndHessian() {
        while (running) {
            ...
            try {
            	if (client == null) {
            		this.client = clientFactory.get(targetIP, targetPort, rpcTimeout);
            	}
                Object response = null;
                response = client.request(msgId,requestObject);
